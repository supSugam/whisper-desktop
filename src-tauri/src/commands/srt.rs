...
/// Rules: Max 42 chars per line, max 2 lines, 2-4 seconds duration
fn merge_tokens_to_subtitles(tokens: Vec<(i64, i64, String)>) -> Vec<SubtitleEntry> {
    ...
    let max_chars_per_line = 42;
    let max_duration_ms: i64 = 4000;
    let min_duration_ms: i64 = 1000;

    let mut current_text = String::new();
    let mut current_start: i64 = tokens[0].0;
    let mut current_end: i64 = tokens[0].1;
    ...
    for (start, end, word) in tokens {
        ...
        let test_text = if current_text.is_empty() {
            word_trimmed.to_string()
        } else {
            format!("{} {}", current_text, word_trimmed)
        };

        let duration = end - current_start;
        let too_long = test_text.len() > max_chars_per_line * 2; // Allow 2 lines
        let too_much_time = duration > max_duration_ms;

        if (too_long || too_much_time) && !current_text.is_empty() {
            // Save current subtitle and start new one
            if current_end - current_start >= min_duration_ms / 2 {
            ...
            current_text = word_trimmed.to_string();
            current_start = start;
            current_end = end;
        } else {
            ...
            if current_text.is_empty() {
                ...
                current_text = format!("{} {}", current_text, word_trimmed);
            }
            current_end = end;
        }
    }
    ...
    if !current_text.is_empty() && current_end - current_start >= min_duration_ms / 2 {
        subtitles.push(SubtitleEntry {
            start_ms: current_start,
            end_ms: current_end,
            text: current_text.trim().to_string(),
...
pub async fn generate_srt<R: Runtime>(
    ...
    let vad_threshold = 0.001; // Energy threshold for speech
    ...
    // Configure Whisper params for timestamps
    ...
    whisper_params.set_token_timestamps(true); // Enable word-level timestamps
    whisper_params.set_no_speech_thold(0.6);
    whisper_params.set_max_len(1); // Force token-level output for precise timestamps

    // Process audio
    state.full(whisper_params, &samples).map_err(|e| format!("Failed to run model: {}", e))?;

    // Emit transcription done
    let _ = app.emit("srt-progress", SrtProgress {
        percentage: 80,
    ...
    // Merge tokens into subtitles
    let subtitles = merge_tokens_to_subtitles(all_tokens);
